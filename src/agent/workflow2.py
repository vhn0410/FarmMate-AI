...
async def planner(state: State, workflow: Workflow):
    ...
    try:
        # Option A: direct call with timeout to prevent hangs
        # Adjust timeout (seconds) to suit your environment
        TIMEOUT_SEC = 60
        result = await asyncio.wait_for(workflow.llm_with_tools.ainvoke(messages), timeout=TIMEOUT_SEC)
        return {"messages": [result]}

    except asyncio.TimeoutError:
        # LLM took too long — return a clear fallback
        print("❌ [planner] Timeout while calling llm_with_tools.")
        fallback = AIMessage(content="Planner: LLM timeout — thử lại sau.")
        return {"messages": [fallback]}

    except asyncio.CancelledError:
        # Client or outer task cancelled. Log and return a short fallback
        # NOTE: we do NOT re-raise to avoid crashing the whole graph stream.
        print("⚠️ [planner] CancelledError caught inside planner (client disconnected or upstream cancel).")
        # Provide a minimal fallback so graph can continue gracefully
        fallback = AIMessage(content="Planner: Tác vụ bị hủy (Cancelled). Vui lòng thử lại.")
        return {"messages": [fallback]}

    except Exception as e:
        # Unexpected errors -> log traceback and return AIMessage explaining error
        tb = traceback.format_exc()
        print(f"❌ [planner] Unexpected error: {e}\n{tb}")
        fallback = AIMessage(content=f"Planner lỗi nội bộ: {str(e)}")
        return {"messages": [fallback]}
